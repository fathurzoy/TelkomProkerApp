...
const IsRoute = ({showTambah, akses}) => {
  ...
  const getStaffList = async () => {
    let datas = await staff_find_many_is(page);
    setStaffListMany(datas);
  };
  const getStaffListLimit = async () => {
    let limit = null;
    let datas = await staff_find_many_is(page, limit);
    ...
    <>
      ...
      <View style={{height: 670}}>
        <View
          ...
          <Text style={{fontSize: 24}}>Staff Is</Text>
          <View
            style={{
              flexDirection: 'row',
            }}>
            ...
            />
            <Gap width={10} />
            <PdfExport dataStaff={staffListLimit} title={'Is'} divisi_id={5} />
          </View>
        </View>
