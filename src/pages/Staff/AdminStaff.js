...
const AdminRoute = ({showTambah, akses}) => {
  ...
  const getStaffList = async () => {
    let datas = await staff_find_many_admin(page);
    setStaffListMany(datas);
  };
  const getStaffListLimit = async () => {
    let limit = null;
    let datas = await staff_find_many_admin(page, limit);
      ...
      <View style={{height: 670}}>
        <View
          ...
          <View
            ...
            />
            <Gap width={10} />
            <PdfExport dataStaff={staffListLimit} title={'Ipn'} divisi_id={4} />
          </View>
        </View>
