...
const CmeRoute = ({showTambah, akses}) => {
  ...
  const getStaffList = async () => {
    let divisi_id = 1;
    let datas = await staff_find_many({page, divisi_id});
    setStaffListMany(datas);
  };
  const getStaffListLimit = async () => {
    let divisi_id = 1;
    let limit = null;
    let datas = await staff_find_many({page, limit, divisi_id});
      ...
      <View style={{height: 670}}>
        <View
          ...
          <View
            ...
            />
            <Gap width={10} />
            <PdfExport dataStaff={staffListLimit} title={'CME'} divisi_id={1} />
          </View>
        </View>
